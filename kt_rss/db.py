...
def upsert_article(conn: sqlite3.Connection, a: Article) -> str:
    ...
    if row is None:
        conn.execute(
            ...
            INSERT INTO articles (
                ...
                published_at, modified_at, is_paywalled, first_seen, last_seen
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (a.id, a.url, a.title, a.subtitle, a.section, a.kicker, a.author,
             a.published_at, a.modified_at, a.is_paywalled, now, now),
        )
        return "inserted"

    # Känd artikel: first_seen rör vi aldrig. Bara title/subtitle/modified_at
    ...
    if changed:
        conn.execute(
            ...
            "last_seen = ? WHERE id = ?",
            (a.title, a.subtitle, a.modified_at, now, a.id),
        )
        return "updated"
...
def get_articles(
    conn: sqlite3.Connection, *, section: str | None = None, limit: int = 50
) -> list[sqlite3.Row]:
    """Artiklar sorterade på published_at fallande, ev. filtrerade på sektion."""
    if section is None:
        return conn.execute(
